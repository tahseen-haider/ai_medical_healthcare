...
function Navbar({ isAuthenticated }: { isAuthenticated: boolean }) {
  ...
  return (
    <header
      className={`fixed w-screen h-16 dark:bg-dark-1 bg-light-4 flex items-center justify-between px-2 lg:px-6`}
    >
      ...
      <nav>
        <ul
          className={`   lg:relative lg:flex-row lg:w-fit lg:top-0 lg:z-0
                        absolute dark:bg-dark-1 bg-light-4 flex flex-col items-center lg:gap-7 w-screen left-0 ${isNavbarDown?"top-16":"-top-60"} transition-all duration-1000 -z-20`}
        >
          {NavLinks.map((ele) => (
      ...
      <div className="flex gap-4 lg:w-37 w-48 justify-end items-center">
        <div className="lg:hidden"><Btn onClick={()=>setIsNavbarDown(!isNavbarDown)}>{!isNavbarDown? <Menu/>: <X/>}</Btn></div>
        {!isAuthenticated && (
          <Link
            href="/login"
